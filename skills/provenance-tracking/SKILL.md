...
2. Diff against previous version (cached in references/.cache/)
3. Classify changes:
   - SEMANTIC: new phases, renamed concepts, added/removed sections,
     changed principles, new toolchain roles, modified exit conditions,
     altered convergence criteria, new anti-patterns
   - COSMETIC: formatting, typo fixes, rewording without meaning change
4. If SEMANTIC changes found:
   a. Update SKILL.md to reflect material upstream changes
   b. Update references/ if upstream adds new reference-worthy content
   c. Preserve all local adaptations (self-learning protocol, project patterns)
   d. Increment version in frontmatter
   e. Record in references/CHANGELOG.md with summary of merged changes
5. If COSMETIC only:
   a. Update last-reviewed date
   b. No SKILL.md changes needed
6. Always update assets/upstream.md via check-upstream.sh --update
```

**Semantic update examples:**

| Upstream Change | Agent Action |
|-----------------|-------------|
| New VDD phase added (e.g., "Formal Hardening") | Add phase to SKILL.md instructions |
| Renamed concept ("beads" → "atoms") | Update terminology throughout |
| New exit condition added | Merge into convergence criteria |
| Toolchain role added/changed | Update toolchain table |
| Core principle added | Add to principles section |
| Anti-pattern added | Merge into anti-patterns table |
| Section restructured significantly | Restructure SKILL.md to match |

**Preservation rules — never remove:**

- Local self-learning / self-updating protocol sections
- Project-specific instructions or patterns
- references/LESSONS_LEARNED.md entries
- Custom justfile recipes or scripts unrelated to upstream
