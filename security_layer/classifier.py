...
def is_injection_pattern(text: str) -> bool:
    ...
    return any(pattern.lower() in normalized for pattern in INJECTION_PATTERNS)
...
def _score_injection(text: str) -> tuple[float, list[str]]:
    ...
    matched = [pattern for pattern in INJECTION_PATTERNS if pattern.lower() in combined]
    if not matched:
        return 0.0, []
